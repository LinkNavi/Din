...
module Heap {
    ...
    uint64 hdr_size(uint64 h)          { return *cast<*uint64>(h); }
    uint64 hdr_flags(uint64 h)         { return *cast<*uint64>(h + 8); }
    void   set_size(uint64 h, uint64 v) { *cast<*uint64>(h) = v; }
    void   set_flags(uint64 h, uint64 v){ *cast<*uint64>(h + 8) = v; }
